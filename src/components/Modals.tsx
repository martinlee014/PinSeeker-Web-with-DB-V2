...
// 1. Single Player Input Component (NEW - Big UI)
const SinglePlayerScoreInput = ({ 
    par, 
    data, 
    onUpdate 
}: { 
    ...
    if (data.totalScore < par) scoreColor = 'text-red-400';
    else if (data.totalScore > par) scoreColor = 'text-blue-400';

    return (
        <div className="flex flex-col items-center justify-center py-4 space-y-8 animate-in zoom-in-95 duration-300">
            {/* Main Score Big Control */}
            <div className="flex items-center gap-8">
                <button 
                    onClick={() => onUpdate('totalScore', -1)}
                    className="w-16 h-16 rounded-full bg-gray-800 hover:bg-gray-700 border border-gray-600 flex items-center justify-center transition-all active:scale-90"
                >
                    <Minus size={32} className="text-gray-400" />
                </button>
                
                <div className="flex flex-col items-center w-32">
                    <span className={`text-8xl font-black leading-none ${scoreColor} drop-shadow-xl`}>
                        {data.totalScore}
                    </span>
                    <span className="text-xs font-bold text-gray-500 uppercase tracking-widest mt-2">Strokes</span>
                </div>
            ...
            <div className="grid grid-cols-2 gap-6 w-full max-w-xs">
                {/* Putts */}
                <div className="bg-gray-800/50 p-4 rounded-2xl border border-gray-700 flex flex-col items-center">
                    <span className="text-xs font-bold text-gray-500 uppercase mb-3">Putts</span>
                    <div className="flex items-center gap-4">
                        <button onClick={() => onUpdate('putts', -1)} className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center active:bg-gray-600"><Minus size={16} /></button>
                        <span className="text-2xl font-bold text-white w-6 text-center">{data.putts}</span>
                        <button onClick={() => onUpdate('putts', 1)} className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center active:bg-gray-600"><Plus size={16} /></button>
                    </div>
                </div>

                {/* Penalties */}
                <div className="bg-gray-800/50 p-4 rounded-2xl border border-gray-700 flex flex-col items-center">
                    <span className="text-xs font-bold text-gray-500 uppercase mb-3">Penalties</span>
                    <div className="flex items-center gap-4">
                        <button onClick={() => onUpdate('pens', -1)} className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center active:bg-gray-600"><Minus size={16} /></button>
                        <span className="text-2xl font-bold text-red-400 w-6 text-center">{data.pens}</span>
                        <button onClick={() => onUpdate('pens', 1)} className="w-8 h-8 rounded-full bg-gray-700 flex items-center justify-center active:bg-gray-600"><Plus size={16} /></button>
                    </div>
                </div>
            </div>
        </div>
...
export const ScoreModal = ({ 
  ...
  const isMultiplayer = Object.keys(allScores).length > 1;
    ...
    <ModalOverlay onClose={onClose}>
      ...
      <div className="p-4 overflow-y-auto bg-gray-900 flex-1">
          {isMultiplayer ? (
              ...
              // SINGLE PLAYER BIG UI
              <SinglePlayerScoreInput 
                  par={par}
                  data={allScores[Object.keys(allScores)[0]]}
                  onUpdate={(field, delta) => updateScore(Object.keys(allScores)[0], field, delta)}
              />
          )}
      </div>
...
// --- Reusable Scorecard Table for Horizontal Scrolling (Group/Tournament Mode) ---
...
// --- Single Player Scorecard (Traditional Format) ---
const SinglePlayerScorecard = ({ holes, scorecard }: { holes: GolfHole[], scorecard: HoleScore[] }) => {
    
    const renderNine = (start: number, end: number, title: string) => {
        // Ensure consistent 9-hole range even if some hole definitions are missing
        const holeRange = Array.from({length: 9}, (_, i) => start + i);

        // Calculate Totals
        const scores = scorecard.filter(s => s.holeNumber >= start && s.holeNumber <= end);
        const totalScore = scores.reduce((sum, s) => sum + s.shotsTaken + s.putts + s.penalties, 0);
        const totalPutts = scores.reduce((sum, s) => sum + s.putts, 0);
        
        // Calculate Par Total (fallback to 4 if par missing)
        const totalPar = holeRange.reduce((sum, num) => {
            const h = holes.find(x => x.number === num);
            return sum + (h?.par || 4);
        }, 0);

        return (
            <div className="mb-6 last:mb-0">
                {/* Traditional Scorecard Layout - Flexbox for stability */}
                <div className="flex flex-col border border-gray-700 rounded-xl overflow-hidden text-xs bg-gray-900">
                    
                    {/* Header Row */}
                    <div className="flex bg-gray-800 border-b border-gray-700">
                        <div className="w-10 p-2 font-bold text-gray-500 text-center flex items-center justify-center border-r border-gray-700">Hole</div>
                        {holeRange.map(num => (
                            <div key={num} className="flex-1 p-2 font-bold text-white text-center border-r border-gray-700 last:border-r-0 border-l border-l-gray-700/50">
                                {num}
                            </div>
                        ))}
                        <div className="w-10 p-2 font-black text-gray-300 text-center bg-gray-700 text-[10px] uppercase tracking-wider flex items-center justify-center">
                            {title}
                        </div>
                    </div>
                    ...
                    <div className="flex bg-gray-900/50 border-b border-gray-700">
                        <div className="w-10 p-2 font-bold text-gray-500 text-center border-r border-gray-700">Par</div>
                        {holeRange.map(num => {
                            const h = holes.find(x => x.number === num);
                            return (
                                <div key={num} className="flex-1 p-2 text-gray-400 text-center border-r border-gray-700 last:border-r-0 border-l border-l-gray-700/50">
                                    {h?.par || 4}
                                </div>
                            )
                        })}
                        <div className="w-10 p-2 font-bold text-gray-400 text-center bg-gray-800/50">
                            {totalPar}
                        </div>
                    </div>
                    ...
                    <div className="flex bg-gray-900 border-b border-gray-800">
                        <div className="w-10 p-2 font-bold text-white text-center flex items-center justify-center border-r border-gray-800">Scr</div>
                        {holeRange.map(num => {
                            const s = scorecard.find(x => x.holeNumber === num);
                            const h = holes.find(x => x.number === num);
                            const val = s ? s.shotsTaken + s.putts + s.penalties : 0;
                            const par = h?.par || 4;
                            ...
                            if (val > 0) {
                                ...
                                if (val < par) color = 'text-red-400 font-black';
                                else if (val > par) color = 'text-blue-400';
                                else color = 'text-white font-bold';
                            }

                            return (
                                <div key={num} className={`flex-1 p-2 font-bold text-center border-r border-gray-800 last:border-r-0 border-l border-l-gray-800 ${color}`}>
                                    {displayVal}
                                </div>
                            )
                        })}
                        <div className="w-10 p-2 font-black text-white text-center bg-gray-800/30">
                    ...
                    <div className="flex bg-gray-900">
                        <div className="w-10 p-2 font-bold text-gray-500 text-center border-r border-gray-800">Putts</div>
                        {holeRange.map(num => {
                            const s = scorecard.find(x => x.holeNumber === num);
                            return (
                                <div key={num} className="flex-1 p-2 text-[10px] text-orange-400 text-center border-r border-gray-800 last:border-r-0 border-l border-l-gray-800">
                                    {s?.putts || '-'}
                                </div>
                            )
                        })}
                        <div className="w-10 p-2 font-bold text-orange-400 text-center bg-gray-800/30">
...
export const FullScorecardModal = ({ 
  ...
  // Logic to determine display mode
  // If we have explicit group scorecards, use them. 
  // If not, fallback to single 'scorecard' prop wrapped in an object.
  const cards = groupScorecards.length > 0 ? groupScorecards : (scorecard ? [{ scorecard, player: 'Me' } as RoundHistory] : []);
  
  // Explicitly check for single player scenario
  const isSinglePlayer = cards.length === 1;

  return (
    <ModalOverlay onClose={onClose} className="max-w-4xl w-full h-[80vh]">
      <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-gray-900 shrink-0">
        <h3 className="text-lg font-bold text-white flex items-center gap-2">
          <Trophy size={18} className="text-yellow-500"/> {isSinglePlayer ? 'Scorecard' : 'Group Scorecard'}
        </h3>
        <button type="button" onClick={onClose}><X className="text-gray-400" /></button>
      </div>
      
      {isSinglePlayer ? (
          <SinglePlayerScorecard holes={holes} scorecard={cards[0].scorecard} />
      ) : (
